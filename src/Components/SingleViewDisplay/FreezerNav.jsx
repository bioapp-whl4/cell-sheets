...
class FreezerNav extends Component {
  ...
  render() {
    let displayFreezers = this.state.freezers.map((elem, i) => {
      return (
        <div
          className="displayFreezers"
          onClick={() => this.getFreezerCane(elem.freezer_id)}
          key={i}
        >
    ...
    return (
      <div className="display">
        <div className="contents">
          ...
          {this.state.cane_id !== null && (
            <div>
              {" "}
              <h4 onClick={() => this.clearBoxes()}>Freezer Boxes</h4>
              {displayFreezerBoxes}
            </div>
          )}
          {this.state.box_id !== null && (
            <div>
              <h4>Box</h4>
              <GridContextProvider box_id={this.state.box_id} />
            </div>
          )}
        </div>
      </div>
