...
class AddLocation extends Component {
    ...
    handleFreezer = async (e) => {
        await this.setState({ [e.target.name]: e.target.value, freezercane: '' })
        this.getFreezerCane()

    }

    getFreezerCane = async () => {
        let res = await axios.get(`/api/freezer/canes?id=${this.state.freezer}`)
        this.props.updateFreezerCanes(res.data)

    }
    render() {
        console.log('this is the STATE', this.state)
        console.log(this.props.freezercanes)
        ...
        return (
            <div>
                ...
                <select name='freezer' onChange={this.handleFreezer}><option value='-1'>Choose a Freezer</option>{freezers}</select>
                <h4>FreezerCane</h4>
