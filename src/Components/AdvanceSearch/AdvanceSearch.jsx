    ...
    try {
      ...
      res1.data.map(async freezer => {
        let res2 = await axios.get(
          `/api/freezer/canes?id=${freezer.freezer_id}`
        );
  ...
  render() {
    ...
    if (this.state.search_value) {
      ...
      results = this.state.samples.filter(sample => {
        let search_within = "";
        search_within +=
          this.state.description && sample.description
            ? sample.description.toLowerCase()
            : "";
        search_within +=
          this.state.experiment_id && sample.experiment_name
            ? sample.experiment_name.toLowerCase()
            : "";
        search_within +=
          this.state.sample_id && sample.sample_name
            ? sample.sample_name.toLowerCase()
            : "";
